...
--     d. **Difficult Bonus:** *Do not attempt until you have solved all other problems!* For each specialty, report the percentage of total claims by that specialty which are for opioids. Which specialties have a high percentage of opioids?
	...
	WITH opioids AS (
		SELECT md.specialty_description, SUM(sc.total_claim_count) AS opioid
		FROM prescription AS sc
			LEFT JOIN prescriber AS md USING (npi)
	...
	FROM prescriber AS md
		LEFT JOIN prescription AS sc USING (npi)
		LEFT JOIN drug AS d USING (drug_name)
		LEFT JOIN opioids USING (specialty_description)
	WHERE opioids.opioid IS NOT NULL
	GROUP BY md.specialty_description, opioids.opioid
...
--     b. Next, report the number of claims per drug per prescriber. Be sure to include all combinations, whether or not the prescriber had any claims. You should report the npi, the drug name, and the number of claims (total_claim_count).
	...
	SELECT npi, d.drug_name, sc.total_claim_count
	FROM prescriber
		CROSS JOIN drug AS d
		LEFT JOIN prescription AS sc USING (npi, drug_name)
	WHERE specialty_description LIKE 'Pain Management'
		AND nppes_provider_city LIKE 'NASHVILLE'
		AND d.opioid_drug_flag LIKE 'Y'
	ORDER BY sc.total_claim_count ASC
	
--     c. Finally, if you have not done so already, fill in any missing values for total_claim_count with 0. Hint - Google the COALESCE function.

	SELECT npi, d.drug_name, coalesce(sc.total_claim_count, 0)
		FROM prescriber
			CROSS JOIN drug AS d
			LEFT JOIN prescription AS sc USING (npi, drug_name)
		WHERE specialty_description LIKE 'Pain Management'
